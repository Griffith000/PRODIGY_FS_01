...
const  googleOAuth = async (req, res, next) => {
  ...
  try {
   const user = await User.findOne({email : req.body.email});
    ...
    const newUser = new User({username: name.split(' ').join('').toLowerCase() + Math.floor(Math.random() *1000).toString() , email ,password: hashedPassword, profilePicture: photoURL});
    await newUser.save();
